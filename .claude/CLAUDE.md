...
5. **Claude 후처리** (`ai_enhance.py`): 6개 병렬 태스크 (chapters/glossary/keypoints/qa/design_brief/entities) + 1회 순차 정규화 패스
...
| `design_brief.md` | ai_enhance | 세션 주제 게임의 비주얼 아이덴티티 브리프 (NotebookLM 슬라이드 테마용) |
| `related_articles.md` | web_context | 해외 게임 언론 관련 기사 (섹션 블록 포맷) |
...
| 개발 서버 | `uvicorn app:app --reload --port 8100` |
| CLI 자막 추출 (기본) | `python gdc_transcript.py <URL>` |
| CLI + AI 후처리 | `python gdc_transcript.py <URL> --enhance` |
| CLI 개별 옵션 | `--chapters --glossary --keypoints --qa --design-brief --articles --thumbnail` |
